        ...
        <div class="row">
            <div class="col-12 col-lg-4 mb-3">
                ...
                <select class="form-control" id="selectType" name="type">
                    <option value="talk" {{old('type') == 'workshop' ? '' : 'selected'}}>Talk</option>
            ...
            <div class="col-12 col-lg-4 mb-3">
                ...
                <select id="inputSpeaker" name="speaker[]" class="form-control {{$errors->first('speaker') ? 'is-invalid' : ''}}" multiple>
                    ...
                    @foreach($speakers as $speaker)
                        <option value="{{$speaker->id}}" {{old('speaker') ? (in_array($speaker->id, old('speaker')) ? 'selected' :'') : ''}}>{{$speaker->name}}</option>
                    @endforeach
                </select>
        ...
        <div class="row">
            <div class="col-12 col-lg-4 mb-3">
                ...
                <select class="form-control {{$errors->first('room_id') ? 'is-invalid' : ''}}" id="selectRoom" name="room_id">
                    ...
                    <option value="{{$room->id}}" {{old('room_id') == $room->id ? 'selected' : ''}}>{{$room->name}} / {{$room->channel->name}}</option>
                        @endforeach
                </select>
                <div class="invalid-feedback">
